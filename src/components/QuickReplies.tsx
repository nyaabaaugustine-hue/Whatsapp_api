...
export function QuickReplies({ replies, onSelect }: QuickRepliesProps) {
  const iconFor = (reply: QuickReply) => {
    ...
    if (t.includes('family')) return <span className="text-[12px]">👨‍👩‍👦‍👦</span>;
    if (t.includes('business')) return <span className="text-[11px]">💼</span>;
    if (t.includes('daily')) return <span className="text-[11px]">🚘</span>;
    ...
    if (t.includes('fuel')) return <span className="text-[11px]">⛽</span>;
    ...
    if (t.includes('power')) return <span className="text-[11px]">🚘</span>;
    if (t.includes('sedan')) return <span className="text-[11px]">🚘</span>;
    if (t.includes('suv')) return <span className="text-[11px]">🚘</span>;
    ...
    if (t.includes('show car') || t.includes('show cars')) return <span className="text-[11px]">🚘</span>;
    ...
    if (t.includes('paying full') || t.includes('cash')) return <span className="text-[11px]">💰</span>;
    ...
    if (t.includes('ghs') || t.includes('under') || t.includes('budget')) return <span className="text-[11px]">💰</span>;
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-2 mb-1">
      {replies.map((reply) => (
        ...
        className="inline-flex items-center gap-2 bg-[#0b141a] hover:bg-[#0f2d25] text-[#dff8f0] px-2.5 py-1 rounded-[6%] text-[11px] font-semibold transition-colors border border-[#25D366] shadow-sm"
      >
          <span className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-full bg-[#25D366] text-[#0b141a]">
            {iconFor(reply)}
          </span>
          {reply.text}
        </button>
